...
#
# Maintained by: Ryan Pannell
#
# Write Linksys signature for factory image
# This is appended to the factory image and is tested by the Linksys Upgrader
# - as observed in civic.
# The footer is 256 bytes. The format is:
#   .LINKSYS.  detected by the Linksys upgrader before continuing with upgrade
#              (9 bytes)
#   version    version number of upgrade, not checked so use arbitrary value
#              (8 bytes)
#   model      model of target device, padded (0x20) (15 bytes)
#   crc        CRC checksum of the image to flash (8 byte)
#   padding    (0x20) (7 bytes)
#   signature  signature of signer, not checked so use arbitrary value
#              (16 bytes)
#   padding    (0x00) (192 bytes)
#   0x0A       (1 byte)
## version history
...
usage() {
	echo "Usage: $ME <type> <input> <output>"
