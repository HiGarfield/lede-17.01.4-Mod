...
function policyWarn() -- display status and warning messages at the top of the page
	if nameTooLong == 1 then
		return "<font color=\"ff0000\"><strong>"..string.format(translate("WARNING: this policy's name is %d characters exceeding the maximum of 15!"),policyNameLength).."</strong></font>"
	else
		return ""
	end
end
...
m5 = Map("mwan3", string.format(translate("MWAN Policy Configuration - %s"),arg[1]), translate(policyWarn()))
